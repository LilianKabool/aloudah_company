...
@injectable
class GetAllActiveTripsPaginatedCubit extends Cubit<PaginationStateTest<dataForArctiveTrips>>  {
  final AdminBaseRepository _adminBaseRepository;
  int currentPage = 1;
  int? lastPage;
  List<dataForArctiveTrips?> _trips = [];
    ...
    response.fold(
        ...
        if (r.data != null) {
          ...
          emit(PaginationStateTest.success(
            data: _trips.where((element) => element != null).cast<dataForArctiveTrips>().toList(),
  ...
  void _addIncomingDataToClassMemberData(
      bool loadMore, BasePaginationEntity<PaginationEntity<dataForArctiveTrips?>> model) {
    if (loadMore) {
