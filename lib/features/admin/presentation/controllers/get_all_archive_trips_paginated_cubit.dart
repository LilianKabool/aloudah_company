...
@injectable
class GetAllArchiveTripsPaginatedCubit extends Cubit<PaginationStateTest<archiveTrip>> {
  final AdminBaseRepository _adminBaseRepository;
  int currentPage = 1;
  int? lastPage;
  List<archiveTrip?> _trips = [];
      ...
      (r) {
        lastPage = r.data!.lastPage;
          _addIncomingDataToClassMemberData(loadMore, r);
                  emit(PaginationStateTest.success(data: _trips.where((element) => element != null).cast<archiveTrip>().toList(), canLoadMore: currentPage));
  ...
  void _addIncomingDataToClassMemberData(
      bool loadMore, BasePaginationEntity<PaginationEntity<archiveTrip?>> model) {
    if (loadMore) {
