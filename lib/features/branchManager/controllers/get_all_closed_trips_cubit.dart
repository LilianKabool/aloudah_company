...
@injectable
class GetAllClosedTripsCubit extends Cubit<PaginationStateTest<ClosedTrip>> {
  final BranchManagerBaseRepository _branchManagerBaseRepository;
  int currentPage = initialPage;
  int? lastPage;
  List<ClosedTrip?> _closedTrips = [];

  GetAllClosedTripsCubit(
  ...
  Future<void> emitGetAllClosedTrips({
    ...
    var response = await _branchManagerBaseRepository.getAllClosedTrips(currentPage);
    response.fold(
          (l) {
        if (kDebugMode) {
          print(l);
        ...
        if (r.data != null) {
          ...
          emit(PaginationStateTest.success(
            data: _closedTrips.where((element) => element != null).cast<ClosedTrip>().toList(),
  ...
  void _addIncomingDataToClassMemberData(bool loadMore, BasePaginationEntity<PaginationEntity<ClosedTrip?>> model) {
    if (loadMore) {
      _closedTrips.addAll(model.data!.data);
    } else {
      _closedTrips = model.data!.data;
    }
  }
}
