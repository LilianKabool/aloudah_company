...
@injectable
class GetAllDriversCubit extends Cubit<PaginationStateTest<DriverPaginatedEntity>> {
  final BranchManagerBaseRepository _branchManagerBaseRepository;
  int currentPage = 1;
  int? lastPage;
  List<DriverPaginatedEntity?> _drivers = [];

  GetAllDriversCubit(
      this._branchManagerBaseRepository,
      ) : super(const PaginationStateTest.loading());

  Future<void> emitGetAllDrivers({
    bool loadMore = false,
  }) async {
    if (loadMore) {
    ...
    var response = await _branchManagerBaseRepository.getAllDrivers(currentPage);
    response.fold(
          (l) {
        if (kDebugMode) {
          print(l);
        ...
        if (r.data != null) {
          ...
          emit(PaginationStateTest.success(data: _drivers.where((element) => element != null).cast<DriverPaginatedEntity>().toList(), canLoadMore: currentPage));
        }
      },
    );
  }

  void _addIncomingDataToClassMemberData(bool loadMore, BasePaginationEntity<PaginationEntity<DriverPaginatedEntity?>> model) {
    if (loadMore) {
      _drivers.addAll(model.data!.data);
    } else {
      _drivers = model.data!.data;
    }
  }
}
