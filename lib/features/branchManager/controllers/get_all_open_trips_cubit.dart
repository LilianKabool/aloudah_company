...
class GetAllOpenTripsCubit extends Cubit<PaginationStateTest<OpenTrip>> {
  ...
  Future<void> emitGetAllOpenTrips({
    ...
    var response = await _branchManagerBaseRepository.getAllOpenTrips(currentPage);
    response.fold(
          (l) {
        if (kDebugMode) {
          print(l);
        ...
        if (r.data != null) {
          ...
          emit(PaginationStateTest.success(
            data: _openTrips.where((element) => element != null).cast<OpenTrip>().toList(),
  ...
  void _addIncomingDataToClassMemberData(bool loadMore, BasePaginationEntity<PaginationEntity<OpenTrip?>> model) {
    if (loadMore) {
      _openTrips.addAll(model.data!.data);
    } else {
      _openTrips = model.data!.data;
    }
  }
}
